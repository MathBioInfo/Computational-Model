clc, clear all
clf;
name = 'fig7_simulations_with_TE';
figname = sprintf('x%s',name);
set(gcf,'position', [128 419 1718 402]);
...
for j = 1:length(tmp)
    ...
    sumdataSim = [sumLysIntact sumLysIncomp; sumInfnIntact sumInfnIncomp; sumBenIntact sumBenIncomp; sumTranIntact sumTranIncomp];
for j =1:2
...
labels =[{'Excision'; 'Re-infection'; 'Beneficial'; 'TEs'}];
subplot(1,3,2)
grey = 0.5*[1 1 1];
...
xlim([0.7, 4.3]);
legend({'Intact', 'Incomplete'},'fontsize',18,'interpreter','latex');
set(gca,'xticklabel',labels(ikeep(order),:))
set(gca,'fontsize',16);
